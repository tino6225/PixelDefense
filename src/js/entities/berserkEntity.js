game.BerserkEntity = me.Entity.extend({
    ...
    init:function (x, y, settings) {
        ...
        if(me.game.berserkSettings === undefined)
            me.game.berserkSettings = settings;
        settings = me.game.berserkSettings;
        settings.image = "newberserkchilled";
        ...
        this.renderable.addAnimation("walkdown", [18, 19, 20, 21, 22, 23, 24, 25, 26],this.animationSpeed);
        this.renderable.addAnimation("walkright", [27, 28, 29, 30, 31, 32, 33, 34, 35],this.animationSpeed);
        this.renderable.addAnimation("walkup", [0, 1, 2, 3, 4, 5, 6, 7, 8],this.animationSpeed);
        this.renderable.addAnimation("walkleft", [9, 10, 11, 12, 13, 14, 15, 16, 17],this.animationSpeed);
        this.renderable.addAnimation("death", [0, 9, 18, 27, 0, 9, 18, 27, {name: 0, delay: Infinity}],this.animationSpeed);
        this.renderable.addAnimation("chilldown", [54, 55, 56, 57, 58, 59, 60, 61, 62],this.animationSpeed*2);
        this.renderable.addAnimation("chillright", [63, 64, 65, 66, 67, 68, 69, 70, 71],this.animationSpeed*2);
        this.renderable.addAnimation("chillup", [36, 37, 38, 39, 40, 41, 42, 43, 44],this.animationSpeed*2);
        this.renderable.addAnimation("chillleft", [45, 46, 47, 48, 49, 50, 51, 52, 53],this.animationSpeed*2);
        ...
        this.power = game.data.berserkerPower;  //Player lives lost on enemy escape.
    ...
    update : function (dt) {
        ...
        if(!this.alive){
            if(this.deathTimer === undefined){
                this.deathTimer = 0;
                if(this.power>0)
                    me.audio.play("berserk_death",false,null,0.5);
                if(!this.inViewport)
        ...
        /*set velocity, x = horizontal (negative is left), y is vertical(negative is up)
         *I belive the * me.timer.tick has to do with syncing to current FPS
         *
         *I modified this to update position depending on velocity and move direction.
         *We should probably tie movement to the value passed to the update function for the reasons here: https://groups.google.com/forum/#!topic/melonjs/t4uaVJJMlr4 */
